...
public static WebElement pcourse(WebDriver driver) 
{
    ...
    /*    
       List<WebElement> colj = driver.findElements(By.xpath("html/body/div[3]/div/div/div[1]/div/div/ol"));
       // List<WebElement> colj = driver.findElements(By.xpath("html/body/div[3]/div/div/div[1]/div[1]/div[2]/h4/span")); 
        for(int j=0;j<colj.size();j++)
        {
        	System.out.println(colj.get(j).getText());
        	
        	
        	
        	Actions actkg = new Actions(driver);
    		actkg.keyDown(Keys.LEFT_CONTROL).keyDown(Keys.LEFT_SHIFT).perform();

    		colj.get(j).click();
    		
    		actkg.keyDown(Keys.LEFT_CONTROL).keyDown(Keys.LEFT_SHIFT).perform();
    		
    		String WindowHandlekg = driver.getWindowHandle();
    		driver.switchTo().window(WindowHandlekg);
      */  	
